...
use diem_crypto::{pqc_kem, pqc_noise};
...
pub struct NoiseStream<TSocket> {
    ...
    session: pqc_noise::PQNoiseSession,
...
impl<TSocket> NoiseStream<TSocket> {
    ...
    pub fn new(socket: TSocket, session: pqc_noise::PQNoiseSession) -> Self {
        Self {
            socket,
            session,
...
enum ReadState {
    ...
    Eof(Result<(), ()>),
    /// Decryption Error
    DecryptionError(pqc_noise::PQNoiseError),
}
...
enum WriteState {
    ...
    Eof,
    /// Encryption Error
    EncryptionError(pqc_noise::PQNoiseError),
}
                            ...
                            .write_message_in_place(&mut self.buffers.write_buffer[..*offset])
                        {
                            Ok(authentication_tag) => {
                                ...
                                self.buffers.write_buffer
                                    [*offset..*offset + pqc_noise::AES_GCM_TAGLEN]
                                    .copy_from_slice(&authentication_tag);
                                // calculate frame length
                                let frame_len = pqc_noise::encrypted_len(*offset);
...
const MAX_WRITE_BUFFER_LENGTH: usize = pqc_noise::decrypted_len(pqc_noise::MAX_SIZE_NOISE_MSG);

/// Collection of buffers used for buffering data during the various read/write states of a [`NoiseStream`]
struct NoiseBuffers {
    /// A read buffer, used for both a received ciphertext and then for its decrypted content.
    read_buffer: [u8; pqc_noise::MAX_SIZE_NOISE_MSG],
    /// A write buffer, used for both a plaintext to send, and then its encrypted version.
    write_buffer: [u8; pqc_noise::MAX_SIZE_NOISE_MSG],
}

impl NoiseBuffers {
    fn new() -> Self {
        Self {
            read_buffer: [0; pqc_noise::MAX_SIZE_NOISE_MSG],
            write_buffer: [0; pqc_noise::MAX_SIZE_NOISE_MSG],
        }
    }
}
...
mod test {
    ...
    use diem_crypto::{pqc_kem, test_utils::TEST_SEED, traits::Uniform as _};
    use futures::{
        executor::block_on,
        future::join,
    ...
    fn dont_read_forever() {
        ...
        let noise_session = pqc_noise::PQNoiseSession::new_for_testing();
    ...
    fn u16_max_writes() {
        ...
        let buf_send = [1; pqc_noise::MAX_SIZE_NOISE_MSG];
        block_on(client.write_all(&buf_send)).unwrap();
        block_on(client.flush()).unwrap();

        let mut buf_receive = [0; pqc_noise::MAX_SIZE_NOISE_MSG];
