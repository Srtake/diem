...
#[derive(Default)]
pub struct AntiReplayTimestamps(HashMap<pqc_kem::PublicKey, u64>);
...
impl AntiReplayTimestamps {
    ...
    pub fn is_replay(&self, pubkey: pqc_kem::PublicKey, timestamp: u64) -> bool {
    ...
    pub fn store_timestamp(&mut self, pubkey: pqc_kem::PublicKey, timestamp: u64) {
        self.0.entry(pubkey)
...
pub struct NoiseUpgrader {
    ...
    noise_config: pqc_noise::PQNoiseConfig,
    /// Handshake authentication can be either mutual or server-only authentication.
    auth_mode: HandshakeAuthMode,
}

impl NoiseUpgrader {
    /// Create a new PQNoiseConfig with the provided keypair and authentication mode.
    pub fn new(
        ...
        Self {
            network_context,
            noise_config: pqc_noise::PQNoiseConfig::new(private_key, public_key),
            auth_mode,
        }
    }
    ...
    pub async fn upgrade_with_noise<TSocket>(
        ...
        remote_public_key: Option<pqc_kem::PublicKey>,
    ) -> Result<(pqc_kem::PublicKey, NoiseStream<TSocket>), NoiseHandshakeError>
    where
    ...
    const PROLOGUE_SIZE: usize = PeerId::LENGTH + pqc_kem::PUBLIC_KEY_LENGTH;

    /// The client message consist of the prologue + a noise message with a timestamp as payload.
    const CLIENT_MESSAGE_SIZE: usize = 
        Self::PROLOGUE_SIZE + pqc_noise::handshake_init_msg_len(AntiReplayTimestamps::TIMESTAMP_SIZE);
    
    /// The server's message contains no payload.
    const SERVER_MESSAGE_SIZE: usize = pqc_noise::handshake_resp_msg_len(0);
        ...
        F: Fn() -> [u8; AntiReplayTimestamps::TIMESTAMP_SIZE],
    {
        ...
        socket.write_all(&client_message)
            .await.map_err(NoiseHandshakeError::ClientWriteFailed)?;
        ...
        let (_, session) = self.noise_config
    ...
    fn authenticate_inbound(
        ...
        if !peer.keys.contains(&remote_public_key) {
            return Err(NoiseHandshakeError::UnauthenticatedClientPubkey(
                remote_peer_short,
                hex::encode(remote_public_key.to_bytes()),
            ));
        }
        Ok(peer.role)
    }
...
mod test {
    ...
    fn build_peers(
        ...
        let (client_private_key, client_public_key) = pqc_kem::keypair();
        let (server_private_key, server_public_key) = pqc_kem::keypair();
    ...
    fn test_handshake_success_server_only_auth() {
        test_handshake_success(false /* is mutual auth */);
    }
    ...
    fn test_handshake_unauthed_keypair_fails_mutual_auth() {
        ...
        client.noise_config = pqc_noise::PQNoiseConfig::new(client_private_key, client_public_key);
